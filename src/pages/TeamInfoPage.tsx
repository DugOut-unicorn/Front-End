    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="relative h-64 bg-gradient-to-br from-blue-900 to-blue-800">
        <div className="relative mx-auto h-full max-w-[1080px] px-6">
          ...
          <div className="absolute bottom-6 left-0 text-white text-sm">
            <div className="text-lg font-bold">2025 시즌 00 위</div>
            <div className="mt-2"><span>경기 수</span><span className="ml-2">0.00</span></div>
            <div>최근 00승 00패</div>
            <div>승률 0.000</div>
          </div>
        </div>
            ...
            <div key={index} className="rounded-lg bg-[#1B2B66] p-6 text-white">
              <div className="flex items-center justify-between">
                <div className="flex flex-col items-center">
                  <img src="/images/lotte_emb.png" alt="롯데" className="mb-2 h-10" />
                ...
                <div className="flex flex-col items-center">
                  <img src="/images/ssg_emb.png" alt="SSG" className="mb-2 h-10" />
      ...
      {/* 팀 순위 섹션 */}
      <div className="mx-auto max-w-[1080px] px-6 mt-10">
        <h2 className="mb-4 text-xl font-bold">팀 내 순위</h2>
        <div className="grid gap-6">
          {["투수", "타자"].map(role => (
            <div key={role}>
              <h3 className="mb-2 inline-block rounded-full bg-gray-200 px-4 py-1">{role}</h3>
              <div className="grid grid-cols-5 gap-4">
                {[...Array(5)].map((_, idx) => (
                  <div key={idx} className="rounded-lg bg-white p-4 shadow">
                    <div className="mb-2 text-sm text-gray-500">스탯 {idx + 1}</div>
                    ...
                    <div className="mt-2 border-t pt-2 space-y-2 text-sm text-gray-600">
                      {[2, 3].map(rank => (
                        <div key={rank} className="flex justify-between">
                          <div className="flex gap-2"><span>{rank}</span><span>임기영</span></div>
                          <span>21.0</span>
                        </div>
                      ))}
                    </div>
      ...
      <div className="mx-auto max-w-[1080px] px-6 mt-10">
        ...
        <div className="flex gap-2 mb-4">
          {["투수", "내야수", "외야수", "포수"].map(pos => (
            <button key={pos} className={`rounded-full px-4 py-1 text-sm ${pos === "투수" ? "bg-blue-900 text-white" : "bg-gray-200"}`}>
              {pos}
            </button>
          ))}
        </div>
          ...
          {[...Array(8)].map((_, index) => (
            <div key={index} className="flex flex-col items-center">
              <img
                src="/images/반즈.png"
                alt="Player"
                className="mb-1 h-[120px] w-[100px] rounded-sm object-cover cursor-pointer hover:scale-105 transition"
